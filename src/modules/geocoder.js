...
// Sanitizes complex strings for Nominatim if IATA lookup fails
function cleanQueryForGeocoding(text) {
  if (!text) return '';
  let cleaned = text;
  // If multiple legs/airports exist (e.g. PEK / PVG), take the first segment
  if (cleaned.includes('/')) {
    cleaned = cleaned.split('/')[0];
  }
  // Remove parenthesized content: (BUD), (PEK), (aktarmalı)
  cleaned = cleaned.replace(/\([^)]*\)/g, '');
  // Clean special characters and collapse extra whitespace
...
function findEmbeddedIata(text, iataTable) {
  ...
  // 1. Match parenthesized 3-letter codes e.g. (BUD), (PEK), (CDG)
  ...
  // 2. Match standalone 3-letter words
...
export async function resolveLocation(query, mode = 'air') {
  ...
  // 2. Embedded IATA match from text string
  ...
  // 3. Nominatim Fallback with Sanitized String (e.g. "ferenc liszt intl. airport")
